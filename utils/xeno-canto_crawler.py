...
class Crawler:
    ...
    def download_sounds(self, save_to=None):
        '''
        dowloads all sound files for responses stored in df after running query()
        and saves them in structure save_to/{gen}/{instance}/file.mp3.
        DEPRECATED. prefer download_simplified()
        '''
        ...
        for index, row in self.df.iterrows():
            if self.verbose:
                print('downloading {} with id {}'.format(str(row.gen), str(row.id)))
            url = 'http:' + row.file
            path = os.path.join(self.root, str(row.gen), str(row.id))
            if not os.path.exists(path):
                os.makedirs(path)
            r = requests.get(url)
            open(os.path.join(path, 'sound.mp3'), 'wb').write(r.content)


        self.df.to_csv(os.path.join(self.root, 'API_responses.csv'), sep='\t')


    def download_simplified(self, save_to=None):
        '''
        ...
        for index, row in self.df.iterrows():
            if self.verbose:
                print('downloading {} with id {}'.format(str(row.gen), str(row.id)))
            url = 'http:' + row.file
            path = os.path.join(self.root, str(row.gen))
            r = requests.get(url)
        ...
        for path, dirs, files in os.walk(self.root):
            for file in files:
                if file.endswith('.mp3'):
                    if self.verbose:
                        print('processing {}'.format(os.path.join(path, file)))
    ...
    def process_sounds_for_vggish(self):
        '''
        processes dowloaded files below self.root after running download_files() as
        required by pretrained VGGish model.
        DEPRECATED
        '''
        if self.df is None:
            self.df = pd.read_csv(os.path.join(self.root, 'API_responses.csv'), sep='\t')
        self.info_df = self.df[['gen', 'id']].copy()
        for path, dirs, files in os.walk(self.root):
            for file in files:
                if file.endswith('.mp3'):
                    ...
                    y *= 32768
                    y = y.astype(np.int16)
                    wavfile.write(os.path.join(path, file.replace('.mp3', '.wav')), rate=22050, data=y)
                    data = vggish_input.wavfile_to_examples(os.path.join(path, file.replace('.mp3', '.wav')))
                    np.save(os.path.join(path, 'chunks.npy'), data)
                    scipy.misc.imsave(os.path.join(path, 'first_chunk.jpg'), data[0])
        self.info_df.to_csv(os.path.join(self.root, 'info.csv'), sep='\t')
    ...
    def spec_chunks(self, spec, path):
        '''
        cuts a given spectrogram spec (ndarray) into random chunks and saves them to path.
        DEPRECATED
        '''
        start_pos = np.random.randint(0, spec.shape[1]-self.len_chunks, self.n_chunks)
        for i, pos in enumerate(start_pos):
            file_name = 'spec_chunk_{}'.format(i)
            chunk = spec[:, pos : pos+self.len_chunks]
            np.save(os.path.join(path, file_name+'.npy'), chunk)
            if self.save_img:
                scipy.misc.imsave(os.path.join(path, file_name+'.jpg'), chunk)

    def make_dataset(self, root=None, queries=None):
        '''
        creates a dataset by making a query to the API, downloading responded files and
        post-processing these files as specified.
        self.root and self.queries are updated according to the resp arguments if not None.
        DEPRECATED
        '''
        if root is not None:
            self.root = root
...
if __name__ == '__main__':
    ...
    #crawler.download_simplified(save_to='../data/largeBirds2')
    #crawler.process_sounds_for_vggish()
